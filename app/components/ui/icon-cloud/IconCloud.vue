...
// Track how many images have been processed
...
function createImageCanvas(url: string, idx: number): HTMLCanvasElement {
...
function computePosition(index: number, count: number): SphereIcon {
...
watch(
  ...
  (newLen) => {
    ...
    if (!images || !newLen) {
      imageCanvasesRef.value = [];
      imagesLoadedRef.value = [];
      imagePositions.value = [];
      processedCount.value = 0;
      return;
    }

    const oldLen = processedCount.value;

    // Append canvases only for new images (no recreating existing ones)
    for (let i = oldLen; i < newLen; i++) {
      imageCanvasesRef.value.push(createImageCanvas(images[i], i));
      imagesLoadedRef.value.push(false);
    }

    // Recalculate ALL positions so the sphere stays uniform
    const positions: SphereIcon[] = [];
    for (let i = 0; i < newLen; i++) {
      positions.push(computePosition(i, newLen));
    }
    imagePositions.value = positions;
